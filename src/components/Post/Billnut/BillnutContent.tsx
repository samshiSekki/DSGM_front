...
const BillnutContent = ({num,setNum}:NumberProps) => {
    ...
    return (
        ...
        <div><InputDiv style={{width:'50px'}}></InputDiv>학년 수강신청과 전학년 수강신청날 모두 신청을 시도하였지만 결국 수강신청을 하지 못하였기에, 혹여나 정정기간에 증원해주실 수 있는지 문의드립니다.</div>
        {state6 == false ? <ButtonStyled onClick={()=>setState6(true)}>+</ButtonStyled> :
             <><ButtonStyled onClick={()=>setState6(false)}>-</ButtonStyled> <TextArea ref={textRef6} onInput={()=>handleResize(6)}></TextArea></>}
        </>
        : num == 1 ?
        <>
